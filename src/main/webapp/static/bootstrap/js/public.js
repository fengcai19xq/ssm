...
function ajaxFileUploadNew(projectImg, projectName, uploadedfile, businessPkid, type) {
	var indexMsg = layer.msg("图片上传中，请稍后...", {icon: 16, time: 0, shade: 0.3});
	$.ajaxFileUpload({
		url : projectName + '/upload/uploadFileNew',
		secureuri : false,
		fileElementId : uploadedfile,
		dataType : 'json',
		data : {
...
function initTableTr(table, array){
	...
	for(var i=0; i<array.length; i++){
		...
		for(var s=0; s<ths.length; s++){
			var data = array[i][ths[s].innerHTML];
			if(data == null){
				data = "";
			}
			htmlStr += "<td>"+ data +"</td>";
